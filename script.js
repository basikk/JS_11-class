...
function initStartCourseButton() {
  ...
  if (startCourseBtn) {
    startCourseBtn.addEventListener("click", () => {
      const firstLessonLink = document.querySelector(
        '.nav-link[data-target="h-01"]'
      );
...
function updateOrionStatusPanel() {
  ...
  if (textHtml)
    textHtml.textContent =
      (htmlTotal > 0 ? Math.round((htmlCompleted / htmlTotal) * 100) : 0) + "%";
  if (textCss)
    textCss.textContent =
      (cssTotal > 0 ? Math.round((cssCompleted / cssTotal) * 100) : 0) + "%";
  if (textJs)
    textJs.textContent =
      (jsTotal > 0 ? Math.round((jsCompleted / jsTotal) * 100) : 0) + "%";
}
...
function initProjectHub() {
  const projectCheckboxes = document.querySelectorAll(
    '#project-hub input[type="checkbox"]'
  );
